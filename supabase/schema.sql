...
create table if not exists public.pre_dig_clearances (
  id uuid primary key default gen_random_uuid(),
  permit_id uuid references public.excavation_permits(id) on delete cascade,
  permit_number text unique not null,
  road_name text not null,
  ...
  requested_depth_m numeric(6,2) not null default 0,
  nearest_utility_depth_m numeric(6,3),
  safe_clearance_margin_m numeric(6,3),
  ...
  field_marking_status text not null default 'not_started',
  latest_field_note text,
  ar_overlay_status text not null default 'limited',
  gpr_required boolean not null default false,
  blockchain_verified boolean not null default false,
  qr_board_ready boolean not null default false,
  twin_snapshot_year integer,
  twin_note text,
  decision_owner text not null default 'Utility Safety Officer',
  decision_note text,
...
create index if not exists idx_pre_dig_clearances_status on public.pre_dig_clearances(status, risk_score desc, updated_at desc);
...
drop trigger if exists set_pre_dig_clearances_updated_at on public.pre_dig_clearances;
create trigger set_pre_dig_clearances_updated_at
before update on public.pre_dig_clearances
for each row execute function public.set_updated_at();
...
alter table public.pre_dig_clearances disable row level security;
...
values
  (
    'pre_dig_defaults',
    'Configurable thresholds and actions for dig-safe clearance recommendations.',
    '{
      "buffer_m":0.4,
      "gpr_trigger_gap_m":0.75,
      "risk_thresholds":{"critical":85,"high":60,"medium":35},
      "decision_owner":"Utility Safety Officer",
      "required_actions":{
        "critical_conflict":"Resolve critical utility overlap before excavation begins.",
        "gpr":"Schedule GPR or physical utility verification before final clearance.",
        "field_marking":"Capture and sync a utility-marking note from the field console.",
        "approval_flag":"Close compliance flags in the permit approval chain.",
        "qr_board":"Publish the QR site board before corridor mobilization.",
        "standard":"Proceed with hand-dig supervision and standard barricading."
      },
      "checklist_owners":{
        "conflict_review":"Utility Safety Officer",
        "field_marking":"Field Supervisor",
        "traffic_notice":"Traffic Desk",
        "qr_board":"Ward Officer"
      }
    }'::jsonb
  ),
  (
    'geo_defaults',
    'Shared location defaults used when operational records are missing optional geography fields.',
    '{
      "default_city":"Indore",
      "unassigned_ward_label":"Ward pending mapping"
    }'::jsonb
  ),
  (
    'branding_defaults',
    'Branding and city metadata for public and department-facing UI surfaces.',
    '{
      "supported_cities":["Mumbai","Delhi","Indore","Pune"],
      "department_node_label":"Municipal Infrastructure Node",
      "platform_name":"RoadTwin Grid",
      "public_tracker_name":"RoadTwin Pulse"
    }'::jsonb
  ),
  (
    'coordination_defaults',
    'Default bundle text and operational labels for excavation coordination.',
    '{
      "department":"Integrated Utility Cell",
      "fallback_road_name":"Shared corridor",
      ...
      "summary_window_label":"Night Shift",
      "standard_emergency_protocol":"Standard multi-utility supervision",
      "urgent_emergency_protocol":"Protocol B: emergency electric/fiber response",
      "notification_plan":"48h citizen advisory, 24h contractor sync, QR poster auto-generated"
    }'::jsonb
  ),
  (
    'work_order_defaults',
    'Department, crew, SLA, and cost defaults for generated work orders.',
    '{
      "complaint":{
        "department":"Road Infrastructure",
        "crews":{"critical":"Rapid Response Crew","standard":"Crew Alpha"},
        "due_hours":{"critical":8,"standard":24},
        "estimated_cost_inr":{"critical":175000,"standard":85000},
        "fallback_road_name":"Citizen-submitted location",
        "fallback_title_suffix":"complaint response"
      },
      "prediction":{
        "department":"PWD Preventive Maintenance",
        "crews":{"critical":"Crew Delta","standard":"Crew Bravo"},
        "due_hours":{"critical":12,"standard":72},
        "default_budget_inr":125000
      },
      "emergency":{
        "department":"Emergency Restoration Cell",
        "crew":"Rapid Response Crew",
        "due_hours":8,
        "estimated_cost_inr":{"critical":180000,"high":110000}
      },
      "signal":{
        "department":"Road Infrastructure",
        "crews":{"critical":"Rapid Response Crew","standard":"Crew Fusion Ops"},
        "due_hours":{"critical":8,"high":18,"standard":36}
      },
      "fleet":{
        "department":"Fleet Triage Cell",
        "crews":{"critical":"Rapid Response Crew","standard":"Crew Camera Ops"},
        "due_hours":{"critical":6,"standard":24},
        "estimated_cost_inr":{"critical":140000,"standard":70000}
      }
    }'::jsonb
  ),
  (
    'field_sync_defaults',
    'Default values for field capture sync into surveys, emergencies, and notifications.',
    '{
      "survey":{
        "health_score":63,
        "condition":"Field sync pending review",
        "confidence":0.79
      },
      "emergency":{
        "protocol_by_type":{
          "gas_leak":"Protocol A",
          "electrical_fault":"Protocol B",
          "burst_main":"Protocol C",
          "road_collapse":"Protocol D"
        },
        "severity_by_type":{
          "gas_leak":"critical",
          "electrical_fault":"high",
          "burst_main":"high",
        ...
        "checklist_owners":{
          "supervisor_review":"Control Room",
          "detour_notice_release":"Traffic Police"
        }
      },
      "notification":{
        "delay_minutes":10,
        "default_radius_m":300,
        "emergency_radius_m":750,
        "utility_audience":"utility_teams",
        "emergency_detour_text":"Rapid field sync requires corridor review."
      }
    }'::jsonb
  )
